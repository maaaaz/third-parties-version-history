...
parser.add_argument('-o', '--output-file', help='Output csv file (default ./winscp.csv)', default=path.abspath(path.join(os.getcwd(), './winscp.csv')))

def from_chocolatey():
    root = fromstring(requests.get('https://chocolatey.org/packages/winscp').content)
    trs = root.findall('.//tr')
    ...
    for entry in trs:
        ...
        if version_entry and date:
            ...
            winscp['date'] = datetime_obj.date().isoformat()
        
            yield release, winscp
