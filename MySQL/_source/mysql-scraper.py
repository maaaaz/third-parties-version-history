...
parser.add_argument('-o', '--output-file', help='Output csv file (default ./mysql.csv)', default=path.abspath(path.join(os.getcwd(), './mysql.csv')))

def from_chocolatey():
    root = fromstring(requests.get('https://chocolatey.org/packages/mysql').content)
    trs = root.findall('.//tr')
    ...
    for entry in trs:
        ...
        if version_entry and date:
            ...
            mysql['date'] = datetime_obj.date().isoformat()
