...
parser.add_argument('-o', '--output-file', help='Output csv file (default ./mremoteng.csv)', default=path.abspath(path.join(os.getcwd(), './mremoteng.csv')))

def from_chocolatey():
    root = fromstring(requests.get('https://chocolatey.org/packages/mRemoteNG').content)
    trs = root.findall('.//tr')
    ...
    for entry in trs:
        ...
        if version_entry and date:
            ...
            mremoteng['date'] = datetime_obj.date().isoformat()
        
            yield release, mremoteng
