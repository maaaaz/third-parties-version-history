...
parser.add_argument('-o', '--output-file', help='Output csv file (default ./mongodb.csv)', default=path.abspath(path.join(os.getcwd(), './mongodb.csv')))

def from_chocolatey():
    root = fromstring(requests.get('https://chocolatey.org/packages/mongodb').content)
    trs = root.findall('.//tr')
    ...
    for entry in trs:
        ...
        if version_entry and date:
            ...
            mongodb['date'] = datetime_obj.date().isoformat()
        
            yield release, mongodb
