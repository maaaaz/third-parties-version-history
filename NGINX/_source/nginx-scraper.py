...
parser.add_argument('-o', '--output-file', help='Output csv file (default ./nginx.csv)', default=path.abspath(path.join(os.getcwd(), './nginx.csv')))

def from_chocolatey():
    root = fromstring(requests.get('https://chocolatey.org/packages/nginx').content)
    trs = root.findall('.//tr')
    ...
    for entry in trs:
        ...
        if version_entry and date:
            ...
            nginx['date'] = datetime_obj.date().isoformat()
