...
parser.add_argument('-o', '--output-file', help='Output csv file (default ./7-zip.csv)', default=path.abspath(path.join(os.getcwd(), './7-zip.csv')))

def from_chocolatey():
    root = fromstring(requests.get('https://chocolatey.org/packages/7zip').content)
    trs = root.findall('.//tr')
    ...
    for entry in trs:
        ...
        if version_entry and date:
            ...
            seven_zip['date'] = datetime_obj.date().isoformat()
        
            yield release, seven_zip
