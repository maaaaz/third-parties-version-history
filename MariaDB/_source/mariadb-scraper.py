...
parser.add_argument('-o', '--output-file', help='Output csv file (default ./mariadb.csv)', default=path.abspath(path.join(os.getcwd(), './mariadb.csv')))

def from_chocolatey():
    root = fromstring(requests.get('https://chocolatey.org/packages/mariadb').content)
    trs = root.findall('.//tr')
    ...
    for entry in trs:
        ...
        if version_entry and date:
            ...
            mariadb['date'] = datetime_obj.date().isoformat()
        
            yield release, mariadb
