...
parser.add_argument('-o', '--output-file', help='Output csv file (default ./php.csv)', default=path.abspath(path.join(os.getcwd(), './php.csv')))

    
def from_phpnet():
    root = fromstring(requests.get('https://www.php.net/releases/index.php').content)
    trs = root.xpath('.//h2')
    
    p_version = re.compile('(?P<version>\d{1,2}\..*)', re.IGNORECASE)
    for entry in trs:
        release = entry.text
        date = entry.getnext().xpath('./li')[0].text.replace('Released: ','')
        ...
        if version_entry and date:
            ...
            php['date'] = datetime_obj.date().isoformat()
        
            yield release, php

def from_chocolatey():
    root = fromstring(requests.get('https://chocolatey.org/packages/php').content)
    trs = root.findall('.//tr')
    p_version = re.compile('(?P<version>\d{1,2}\..*)', re.IGNORECASE)
