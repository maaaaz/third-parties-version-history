...
parser.add_argument('-o', '--output-file', help='Output csv file (default ./jetty.csv)', default=path.abspath(path.join(os.getcwd(), './jetty.csv')))

def from_chocolatey():
    root = fromstring(requests.get('https://chocolatey.org/packages/jetty').content)
    trs = root.findall('.//tr')
    ...
    for entry in trs:
        ...
        if version_entry and date:
            ...
            jetty['date'] = datetime_obj.date().isoformat()
